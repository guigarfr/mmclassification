"""Transformation stage of OCR recognition model."""
import numpy as np
...
class TPS_SpatialTransformerNetwork(nn.Module):
    ...
    def __init__(
        ...
        ibatch_I: Batch Input Image [batch_size x I_ch_num x I_height x I_width]
        I_size : (height, width) of the input image
        I_r_size : (height, width) of the rectified image I_r
        I_channel_num : the number of channels of the input image
    ...
    def forward(self, batch_I):
        ...
        batch_I_r = functional.grid_sample(
            batch_I,
...
class LocalizationNetwork(nn.Module):
    ...
    predicts C' (K x 2) from I (I_width x I_height)
    """
    ...
    def __init__(self, F, I_channel_num):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(64, 128, 3, 1, 1, bias=False), nn.BatchNorm2d(128),
            nn.ReLU(True),
    ...
    def forward(self, batch_I):
        ...
        input:
        batch_I : Batch Input Image [batch_size x I_channel_num x
        I_height x I_width] output:    batch_C_prime : Predicted coordinates
        of fiducial points for input batch [batch_size x F x 2]
        """
...
class GridGenerator(nn.Module):
    ...
    def _build_P(self, I_r_width, I_r_height):
        """Compute set of points P around the text in the input image.

        Dictate how the image is going to be modified to be straight.
        https://arxiv.org/pdf/1603.03915.pdf 3rd page
        """
        # self.I_r_width
    ...
    def _build_P_hat(self, F, C, P):
        """Build set of points ^P' around the text in the rectified image.

        Will be used to compute P in order to know how the image is going to be
        distorted to have the text straight:
        https://arxiv.org/pdf/1603.03915.pdf 3rd page
        """
    ...
    def build_P_prime(self, batch_C_prime):
        ...
        batch_C_prime_with_zeros = torch.cat(
            (
                batch_C_prime,
                torch.zeros(batch_size, 3, 2).float().to(self.device)
            ),
            dim=1
